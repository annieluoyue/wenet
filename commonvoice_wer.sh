...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  python3 recognize.py \
      ...
      --ctc_weight 0.5 \
      --reverse_weight 0.0 \./utils/test/tex
      --result_file /home/luo/asr/wenet/examples/aishell/s1/exp_gpu/decoder/text 
./utils/test/text/ aishell1
/data/wenet/examples/aishell/s1/utils/test/ aishell2018
fi

if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  echo "stage 6 compute wer"
python3 tools/compute-wer.py --char=1 --v=1 /data/experiment/wenet/examples/aishell/s1/utils/commonvoice_test/text ./decoder/220_CV_text
fi
